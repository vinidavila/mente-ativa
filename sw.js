...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith(fetch(e.request).then(r=>{const cp=r.clone();if(new URL(e.request.url).origin===location.origin)caches.open(CACHE).then(c=>c.put(e.request,cp)); /* inclui audio/*.mp3 conforme forem ouvidos */return r;}).catch(()=>caches.match(e.request).then(r=>r||caches.match('./index.html'))));
});
